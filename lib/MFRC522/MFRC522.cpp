// ─────────────────────────────────────────────────────────────────────────
//  TraceQ 벤더 사본 (miguelbalboa/rfid 1.2.0) — 2026-08-11
//
//  변경 이유: 원본 1.2.0 은 트랜잭션 API 로 옮기면서 구 divider 상수를
//  SPISettings 의 **클럭(Hz) 자리**에 그대로 넘겼다.
//      SPISettings(SPI_CLOCK_DIV4, ...)  //  SPI_CLOCK_DIV4 == 0x00
//  clock=0 이면 Arduino SPI 라이브러리가 최저 분주(fosc/128)를 고르므로
//  16 MHz 보드에서 **실동작 125 kHz** 가 된다(보드 레지스터로 실측 확정).
//  상위 버전에서 MFRC522_SPICLOCK 으로 고쳐진 알려진 버그.
//
//  변경 내용: 4곳의 SPI_CLOCK_DIV4 → MFRC522_SPICLOCK (platformio.ini 에서
//  Hz 로 주입, 미정의 시 아래 기본값). 그 외 원본 코드는 손대지 않았다.
//  라이선스: UNLICENSE (동봉) — 수정·배포 자유.
// ─────────────────────────────────────────────────────────────────────────
#ifndef MFRC522_SPICLOCK
#define MFRC522_SPICLOCK 1000000UL
#endif
/*
