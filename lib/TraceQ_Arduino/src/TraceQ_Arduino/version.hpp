#pragma once
/**
 * \brief TraceQ Arduino 펌웨어 버전.
 *
 * 2.0.0 — 1.0(=1.4.1) 코드베이스를 분석해 RFID 안전성/속도를 재설계 (미완).
 * 2.1.0 — 1.4.1 전 기능 완성판: legacy 서버 프로토콜 전체 이식, raw 시리얼
 *         수신 복원(현장 PC 호환), 소독기 RTC 자동 복구 원복, 태그 발급
 *         키 전환(공장↔TraceQ) 완성, 1.4.1 승계 버그(스택 오버런 6곳,
 *         소독기 매니저 인스턴스 불일치 등) 수정. nested auth 수정 실기 합격.
 * 2.2.0 — 서버 Latest 갈래 삭제(사용자 확정): 서버는 레거시 블록 덤프
 *         단일 경로. Version(Latest/Old) 메뉴·EEPROM 플래그(주소 0)·latest
 *         스텁 제거, 부팅 PSOk 는 서버 타입이면 항상 송신.
 * 2.2.1 — LCD 날짜/시간 저장 시 범위 검증 (월 1~12·일 1~31·시 0~23·분초
 *         0~59) — 1.0부터 있던 월 99 등 쓰레기 값 RTC 저장 구멍 보강.
 *         잘못된 값은 저장하지 않고 Invalid Date/Time 경고 후 폐기.
 * 2.2.2 — RTC 배터리 방전 시 2026-01-01 00:00:00 고정 (1.0은 컴파일 시각).
 *         고정 날짜 = 배터리 방전 식별 마커 + 소독기 RTC 자동 복구 확실 발동.
 * 2.2.3 — 새 빌드 업로드 시 EEPROM 완전 초기화(설정값 포함, 사용자 확정).
 *         빌드 도장(컴파일 시각 해시, 주소 4088) 불일치 = 새 펌웨어 첫 부팅
 *         → 전체 소거+기본값. 1.0의 DATA_NEEDS_INIT(4095) 방식은 구버전이
 *         깔려 있던 기기에서 초기화를 건너뛰어 이중 업로드가 필요했다.
 * 2.2.4 — 초기화 시 액교환일(ClearDateTime) 기본값 = 현재로부터 1개월 전
 *         (사용자 확정). 비어 있으면 소독 기록마다 현재시각이 교환일로
 *         찍혀 통계 주기가 흩어지던 것 방지 — 클리어 태그 등록 전까지
 *         안정된 기준일 제공.
 * 2.2.20 — 재검증 5회차(Z1 Y봉합 재추적 · Z2 시리얼 경합 + 현장 항목): P1 0 · P2 1 · P3 7.
 *         [회귀 2 — 내 v2.2.19 봉합이 만든 것] ① SetData 가 표지를 먼저 내리자 표지 거짓 + 반쪽 키·이름이 남는데
 *           load_manager_data 가 표지를 안 봐 **종료 담당자 블록에 반쪽이 기록**됐다 → 읽는 쪽이 HasData() 를 먼저 본다.
 *           ② 같은 담당자를 다시 댈 때마다 표지가 1→0→1 로 2회 닳았다(일회성 ON 이면 약 5년) → 바뀔 때만 쓴다.
 *         [P3] 미룸을 횟수보다도 먼저(횟수만 써지고 끊기면 액교환이 기록에서 사라졌다) · 기본값 기록의 유효 표지를
 *           타입 하나로(DeviceOption::Upload 는 타입을 마지막에 · deviceOption.Upload() 를 전체 마지막으로) ·
 *           부팅 교환일 두 자리도 미룸 먼저.
 *         [P2] 게이트웨이 수신 관문 — RX 링 511바이트 절단의 꼬리를 받아들여 검사항목을 빈칸으로 기록하거나(성공음)
 *           검사일시 0 으로 기록했다 → 레코드는 머리(G1·G2) 경계에서 시작하고 G5 로 끝나야 쓴다. G1 이 없는 버퍼는
 *           마지막 G2 를 레코드 시작으로(종전엔 첫 환자를 골랐다).
 *         [P3] 511 에서 끊긴 전문의 꼬리를 명령으로 읽어 값 한가운데의 C/M/S 가 발급으로 실행됐다(사장님 허가 09-27)
 *           → 앞 읽기가 한도에서 끊겼으면 그 버퍼는 버린다(와이어 형식 불변).
 *         현장 확인 목록 7 중 4 를 시뮬로 닫음(LCD 잔상·출입카드+스코프 동시·반만 바뀐 태그 재발급·SELECT 길게 누름)
 *           — 두 장 카드 모델·LCD 20x4 표시 격자를 제품 시뮬로 이관 · t_a10(25) 신설 · 변이 4/4 빨강.
 *         변이 17/17 빨강(무해 5 초록) · 354/354(19종).
 * 2.2.19 — 재검증 4회차(Y1 v2.2.18 diff 재추적 · Y2 정수 형제 전수 + 전원 차단 훑기): 제품 P3 4 · 판정 1.
 *         뿌리 하나 — **연속된 EEPROM 쓰기에 원자성이 없다**(바이트당 3.3ms · 중간에 전원이 끊기면 찢긴 값이 남는다).
 *         [P3] 클리어 태그: 교환일 8바이트가 찢겨 '옛 일 + 새 월' 이 남고 그 뒤 소독 기록·태그에 실렸다 →
 *           교환일을 쓰기 전에 미룸을 먼저 세운다(절단되면 다음 부팅이 다시 쓴다).
 *         [P3] 담당자 등록: 표지를 먼저 올려 반쪽 담당자(새 키 + 옛 이름)가 '등록됨' 으로 기록에 실렸다 →
 *           SetKey/SetName 을 SetData 하나로, 표지는 바이트를 다 넣은 뒤 한 번만(절단되면 담당자 없음 · 다시 대면 복구).
 *         [P3] EEPROM 소거 중 리셋: 도장이 그대로라 다음 부팅이 묻지도 않고 반쯤 지워진 설정(타입 W·번호 0)으로 기동 →
 *           소거 loop 앞에서 도장을 무효로.
 *         [판정·재론 금지] 미룸 해소 전 절단은 교환일이 재부팅 시각(순서를 바꾸면 날짜가 아예 안 남는다).
 *         시험: 전원 차단 주입을 제품 시뮬로 이관 · t_a9(10, 절단점 전수 훑기) · t_a8 경계 1 · 변이 7/7 빨강 · 327/327.
 * 2.2.18 — 재검증 3회차(X1 v2.2.17 diff 재추적 · X2 새 각도 전체 재통과): 제품 P1 0 · P2 0 · P3 1 · 판정 2.
 *         [P3] 소독 횟수 32767 에서 +1 이 int16 을 돌아 0 → "MaxCount Over" 가 풀림 → 상한에서 멈춤.
 *         [판정·재론 금지] 동기된 소독기도 앞선 세척기를 따라감(1.0 · 세척기가 유일한 운용 중 시각 기준) ·
 *           두 태그 겹침+HaltA 유실의 재처리는 2칸 이력(상태 증가)으로 막지 않음 · 자정 편집 중 큰 보정은 입력만으로 못 가름.
 *         X1 이 내 잠금 구멍 셋(자정 새 날 시각·is_valid ③·죽은 스크립트)을 잡아 t_a8 에 보강. 변이 4/4 빨강 · 316/316.
 * 2.2.17 — 재검증 2회차(W1 diff 재추적 · W2 새 각도 전체 재통과): P1 1(두 갈래 독립 발견) · P3 4.
 *         [P1] 시간 메뉴의 자정 보정(v2.2.15 내 수정)이 "지금과 12시간 넘게 차이" 로 판정해, 방전된 시계를 처음 맞추는
 *           정당한 조작(1월 1일 → 날짜 저장 → 14:30 입력)까지 하루 옮겨 전날 날짜가 됐다 → 편집 중 **날짜가 바뀐 경우에만**
 *           (진입 스냅샷 ≠ 지금) 진입 날짜와 지금 날짜 중 지금과 가까운 쪽. 자정 넘김·같은 날 큰 보정 둘 다 맞다.
 *         [P3] 세척·소독 is_valid 3진값(1/0/-1) — 이동 플래그는 거부(0)에만 내림(담당자 미등록 상태의 첫 읽기 실패를
 *           거부로 오인하던 조건식 제거) · 게이트웨이 본체번호 4자리는 숫자만 5칸 · 재론 금지 주석 전제 정정 2(알람 0분은
 *           설정기 spin 1~120 라 PC 로는 불가 · 복구 추정 세척시간은 1.0 에 없음) · 창 밖 Reinitialize 재처리 재론 금지.
 *         시험 t_a8(23, t_a7 에서 분리 — 시뮬 RAM 관문) · 변이 3/3 빨강 · 311/311.
 * 2.2.16 — 사장님 판정 둘(09-26): ① 처리 직후 알림음(≤1.6초) 동안 다른 태그로 바꿔 올리면 그 태그도 처리 — "올려 둔
 *           태그 1회만" 을 prev 가 아니라 마지막 처리 UID 로 판정(같은 태그는 KeepAlive, 다른 태그는 Connected).
 *           3차 P3 "2폴≈100ms" 전제가 틀렸던 것(창은 알림음 포함 최대 1.6초). ② 게이트웨이는 세척·소독 과정에 들어간
 *           스코프(WS 또는 DS 표시)를 거부 — 종전엔 둘 다 선 것만 막아 "세척만 하고 소독 안 한" 스코프에 새 환자가 붙었다.
 *           문자열 "No Complete" 그대로. 시험 t_a7 46 · 변이 2/2 빨강 · 305/305.
 * 2.2.15 — 5차 재검증(09-26 밤 · 4갈래: diff 재추적·모듈 관계·전체 재통과 A/B): 남아 있던 것 전부.
 *         [P1] 더블터치 판정(started_just_now)이 시작 블록을 못 읽으면 조용히 '2초 밖' → 종료 확정 → 1초짜리 종료
 *           기록·알람 해제·성공음 → 판정 불가(-1)는 Read Error 로 알리고 아무것도 안 바꾼다.
 *         [P1·조건부] 출시일을 27 로 올렸는데 빌드 날은 26 — 오늘 설치하면 전 기기가 '미동기'. 26 으로 되돌리고
 *           __DATE__ 와 대조하는 static_assert 로 미래 출시일이면 빌드가 실패한다.
 *         [P2] 이동 플래그가 Read Error 에 소모(담당자 일회성의 형제 — 5차가 반만 고침) → 거부에만 · 홈에서 SELECT 를
 *           0.6초 넘게 누르면 기기번호 편집으로 직행(손 뗄 때까지 대기) · 시간 메뉴 저장이 자정을 끼면 +24h(±1일 보정).
 *         [P3] 기기번호는 15열 5칸 고정(자릿수 줄어도 잔상 없음)·상한 999 · "Scope : 32767" · 레거시 발급이 검사일시·
 *           검사명 블록도 비움 · 177번지 손상값 매 부팅 정리 · MaxCount 제목 4자리 · sizeof(Process)==9 static_assert ·
 *           재론 금지 주석 전제 정정 3(RTC begin·알람 0분·doxygen) · 문서 날짜(실제 09-26).
 *         재론 금지: 알람 절대 시각(시계 바꾸면 어긋남) · 소독 시작 중 세척 블록 읽기 실패의 'Write Error' 문구.
 *         시험 t_a7 41 · 되돌린 변이 10/10 빨강(출시일은 빌드 실패) · 298/298.
 * 2.2.14 — 5차 P3 마무리(09-26 밤): 남은 P3 43건 전부 처리 — 고침 11 · 코드에 재론 금지 31 · 소리 규칙 위반 되돌림 1.
 *         [고침] 기기번호·스코프번호 ≥100 표시 · 미인증 서버에 스코프 → 거부음(사장님 통일) · 최대 횟수 안내가 환자정보
 *           경고를 가림 · 폴백 'Not Patient Info' 를 기록 뒤에 송신 · "device_type":"" 이 W 로 바뀌며 재시작 · 파싱 실패
 *           발급의 4초 대기 · ClearSector(≥16)·Screen 0줄 방어 · 거짓 헤더/주석 3.
 *         [재론 금지] 죽은 함수·호출자 0·READER_MODE·범위 밖 무통지·select default·EEPROM 손상 복구·VerifyMismatch 이름·
 *           SimpleScanner·재이동·Status==2·알람 슬롯 하나·이동+방전·복구 세척시간·담당자 0·read_tag·complete_delay·
 *           G1 8자리·중첩 JSON·null·날짜만·Z+명령·48 인덱스·중복 읽기·end+1·2자리 연도·RTC begin·회사코드 재시도·
 *           Invalid Date 320ms(소리 규칙에 묶임).
 *         시험 t_a7 +5 · 되돌린 변이 6/6 빨강 · 288/288.
 * 2.2.13 — 5차 전체 로직 추적 감사(09-26): 모듈 6갈래가 함수 219개를 줄 단위 표로 추적 + 독립 재추적.
 *         [태그 기록] 동시소독에서 슬롯 없는 스코프의 종료가 host 슬롯을 지워 다음 스코프가 guest 대신 host 로
 *           기록되던 것 · 커밋됐는데 확인 읽기만 실패하면 'Write Error'(→ 재접촉이 4초 종료·횟수 0) → 한 번 더
 *           읽어 커밋됐으면 성공 · 일회성 담당자를 실패한 시작·종료 터치가 소모하던 것 · 손상 시작 기록(연도 0xFFFF)이
 *           RTC 를 2047 로 · 환자정보 있던 태그에 새 환자 쓰다 실패 → Status=1+혼합(먼저 내리고 옛 블록 소거).
 *         [게이트웨이] 값 속 G1/G4/G5 를 마커로 집어 환자정보 유실 → 마커는 필드 경계에서만(맨 앞·';' 뒤·G1/G5 뒤) ·
 *           G2 범위 검증(월 13 이 RTC 를 2027 로) · 레거시 발급 번호 -1 금지.
 *         [RFID] 비지원 카드 뒤 곧바로 잡힌 스코프가 KeepAlive 로 먹히던 것(present 잔존) · 발급 도중 전원 상실 태그가
 *           영구 불가(접근비트 이미 TraceQ 면 skip) · 마지막 쓰기 실패 뒤 재인증 낭비·원인 가림 · millis 랩.
 *         [메뉴·시계] 날짜·시간 저장이 메뉴 진입 시각을 씀(편집 시간만큼 시계 되돌아감) · 붙은 버튼이 60초 시한을
 *           영영 미룸(에지에서만 활동) · 선택 화면이 늘 첫 항목부터라 그냥 저장하면 값이 바뀜(타입 D→게이트웨이 재시작!) ·
 *           제목 버퍼 닫는 괄호 잘림 · 알람 줄 20자 넘침·잔상 · 부팅 선택 <> 동시 표본.
 *         [설정] 유지 업그레이드에서 교환일이 비어 있으면 1개월 전(사장님 09-27) · 세척·소독기의 처리 못 하는 태그는
 *           거부음(사장님 09-27) · 알람 슬롯 1·2 만 · 교환일 '빔' 판정 필드로 · 문자열 경계 둘 · 담당자 JSON NUL.
 *         재론 금지(코드에 박음): 더블터치 2초 · 이동 플래그 수명 · ClearSector(5)(6) · 알람 0분 · 둘째 JSON · 폴백 Status 먼저.
 *         시험: t_a7(24) · 되돌린 변이 22/22 빨강 · 독립 재추적 P1/P2 0(성공 경로 34단계 바이트·와이어 문자열 HEAD 동일).
 *           장치: 트레일러 011 은 KEY_B 로만(MF1S50 표7)·buttons_hold·픽스처 접근비트. 283/283.
 * 2.2.12 — 4차 P3 마무리(09-26): 값·표시가 조용히 틀리던 네 자리.
 *         [소독] 쓰이지 않는 SECTOR14 읽기 제거 — 그 읽기가 실패하면 시작이 취소됐는데 앞에서 이미 옛
 *           기록을 지워 "옛 기록 없음 + 새 시작 없음" 태그가 남았다(10바이트를 아래에서 전부 덮어쓴다).
 *         [설정] 게터가 세터 범위 밖 값을 돌려주지 않는다 — 손상·구판 바이트가 음수면 자동 종료가 시작보다
 *           앞선 시각으로 태그에 찍혔다. 세척·소독시간(0~127)·기기번호·소독 횟수·동시소독 슬롯(0~2)·지연(0~120).
 *         [표시] 알람 남은 시간을 절대 시각 차로 — 자정 넘김에서 음수, 60분 초과 설정에서 나머지만 보였다.
 *         [서버] 폐기 블록(18·52) 소거가 실패하면 옛 바이트 대신 0 을 보낸다(계약 "Clear 후 0 덤프").
 *         시험: t_a6(6). 되돌린 변이 5/5 빨강. 261/261.
 * 2.2.11 — 4차 감사 후속(09-26): 서버 연결 확인이 PC 명령을 먹던 것 + 정본 통합 둘 + 첫 부팅 진행 표시.
 *         [서버] PSOk 뒤 'Z' 를 peek 로 본다 — 종전엔 10ms 마다 1바이트 소비해 줄 서 있던 설정 JSON·발급
 *           명령을 먹었고 55바이트 넘으면 정상 연결인데 'Not Connected'. 이제 명령이 앞이면 접촉을 거부하고
 *           다음 loop 의 serialEvent 가 그 명령을 처리한 뒤 재접촉이 성공한다(유실 0).
 *         [정본] 소독기 담당자 키·이름 4줄 직접 쓰기 → RecordProcessor 정본 호출 · Write/WriteBlocks 두 벌이던
 *           재시도·검증 루프 → writeVerified 하나(검사명 3블록·환자 소거 2블록 경로가 처음으로 잠김).
 *         [부팅] 첫 부팅 EEPROM 전체 소거(≈14초)에 "Initializing… %" 진행 표시.
 *         시험: t_a5(12) — peek·정본·guest 대입(2.2.5 M46)·검사일시 이월 금지(2.2.5 M45)·번호 읽기 실패 시
 *           직전 번호 전송 금지. t_notify 서버 훑기의 항상 참 CHECK 해소. 되돌린 변이 6/6 빨강. 253/253.
 * 2.2.10 — 4차 전체 로직 감사(09-25~26): 환자 기록이 조용히 어긋나던 자리 12곳 + 메뉴 시한 + 화면 갱신.
 *         [태그 재처리] 리더 재초기화가 올려 둔 태그 표시를 지워 세척·소독이 곧바로 '종료' 로 기록되던 것 ·
 *           지원 안 되는 카드(교통·출입)를 정지 안 해 그 뒤 스코프가 무음으로 먹히던 것.
 *         [게이트웨이] 환자정보 한 벌은 같은 레코드에서만 — 구간 하나 빠진 패킷에 직전 환자가 남던 것 ·
 *           한 버퍼에 여러 레코드면 마지막(최근) 레코드만, 잘렸으면 폴백(다른 환자 조각을 섞지 않는다).
 *         [커밋 순서] 서버는 완료 커밋 먼저 → 소거 뒤(사장님 09-25: 2차 덤프가 1차와 같아 PC 기록 보호) ·
 *           자동 종료를 커밋 앞으로(실패하면 성공음 없음) · 종료 기록은 시각을 마지막에 · 소독기 이동은
 *           종료 기록 뒤 커밋 · 발급 중 소거 실패는 성공이 아니다.
 *         [설정] 알 수 없는 기기 타입을 매번 재시작하던 것 · 없는 키를 0 으로 저장하던 것(8곳).
 *         [메뉴] 60초 무조작이면 저장 없이 홈으로(사장님 09-25) — 메뉴에 갇혀 태그를 못 읽는 일이 없다.
 *         [화면] 홈 1행·R-O 는 바뀐 것만 그린다(루프 56→27ms, 표시 동일). 문자열 서식 PSTR.
 *         시험: t_a4(15)·t_lcd(6)·t_menu(7) 신설, 각 수정 되돌리면 빨강(음성대조 전수) · 장치에 nackBlock(카드는
 *           살리고 블록만 실패)·buttons_at(시각 지정 버튼)·거짓 초록 3경로(멈춤·요약 없음·CHECK 0) 차단.
 * 2.2.9 — 실패를 조용히 넘기지 않는다 + 소리 규칙 통일 + 업로드 시 설정 유지/초기화 선택 (사장님 09-23).
 *         [알림] 세척·소독 종료·소독 이동·게이트웨이(본 경로/폴백)·서버(덤프 뒤 소거)·담당자 등록·
 *           태그 읽기까지, 기록이 안 됐는데 완료음이 나던 자리를 전부 막았다. 실패하면 LCD 에
 *           Write/Read Error 를 띄우고 실패음을 내며 알람·커밋·성공음이 없다.
 *         [소리] 부저는 한 소리뿐이라 리듬으로 가른다 — 정상=짧게1 · 환자정보없음(기록은 됨)=길게2 ·
 *           실패(다시 대면 됨)=짧게4 · 거부(여기선 안 되는 태그)=삐삐+긴삐. 기기·단계와 무관하게 같은 뜻.
 *         [서버] 덤프 뒤 초기화 순서 재정의: 환자정보표시 내림 → 게이트웨이·검사항목 소거 → 완료 커밋 →
 *           환자 블록 소거. 중간에 멈춰도 "환자정보 있음인데 블록은 빈" 태그가 남지 않고, 2차 덤프가
 *           빈 환자정보로 PC 기록을 덮어쓰지 않는다. 성공 결과는 1.4.1 과 바이트 동일.
 *         [업로드] 쓰던 기기면 첫 부팅에 "Keep settings?" 를 묻는다. 조작은 메뉴와 같다 — `<` `>` 로 고르고
 *           MENU 로 확정(10초 무응답=유지). 공장 기기는 묻지 않고 초기화. 전원 켤 때 RIGHT 을 누르고 있으면 같은 판에서도
 *           다시 고를 수 있다. 유지하면 1.4.1 과 EEPROM 주소가 같아 기기번호·세척시간·담당자·소독 횟수·
 *           액교환일이 그대로 남는다(177번지의 알 수 없는 값만 정리).
 * 2.2.8 — 3차 전체 로직 감사 반영(docs/AUDIT_2026-09-23.md) — 2.2.7 이 넣은 결함 3 포함. 상태를 늘리지 않고 고쳤다:
 *         · 액교환일 미룸의 "시계 맞춰짐" 판정을 RAM 플래그 → 시계 값(출시일 이전 = 미동기)으로.
 *           RAM 플래그는 PC 접속(DTR 리셋)·재시작·시간만 맞춤에 지워져 표지 시각이 교환일로 박혔다.
 *         · 더블터치 판정을 RAM 의 마지막 시작 → 태그에 적힌 시작 시각으로(Write Error 뒤 1초 종료·재부팅 사이 대응).
 *         · 더블터치 재시작은 커밋된 시작을 지우지 않음(중간 실패 시 시작 0 태그).
 *         · 발급 대기는 WUPA 로 정지된 태그도 깨움 · raw 머리글자(G/C/M/S/Z)면 JSON 판정 생략 · str_atoi 오버플로 검사 정정.
 *         · 시뮬레이터 하네스: 진짜 리셋(hard_reset)·WUPA·readErrSkip, 시험 115.
 * 2.2.7 — 2차 전체 로직 감사 반영(docs/AUDIT_2026-09-22.md). 핵심:
 *         · 소독기 RTC 복구에서 세척 종료 기록이 비면 "세척 시작+설정 세척시간+1분"
 *           (종전엔 0 에 +1분 해 unixtime 이 돌아 RTC 가 2043년에 굳었다).
 *         · 액교환일: 시계가 방전 표지(2026-01-01)면 미뤘다가 시계가 맞춰질 때 기록.
 *         · 세척·소독 시작 기록 실패를 성공으로 알리지 않음(Write Error).
 *         · 더블터치 2초 판정을 "방금 시작한 그 태그"로(guest 포함), 세척기에도 적용.
 *         · Poll 3회 무응답만 이탈(인증 실패·HaltA 유실 뒤 재처리 방지), 발급은 ForgetTag.
 *         · 서버 덤프에 읽기 실패가 있었으면 태그를 지우지 않음(재스캔 복구).
 *         · 메뉴 숫자 전 자릿수 저장·날짜 월말 검증·JSON 값 클램프 뒤 좁히기·메뉴 재귀 제거.
 *         · 게이트웨이 수신 대기 1초(올눈 250ms 조각 전송), G3·G4 필드 16바이트 전부 저장.
 * 2.2.6 — 게이트웨이 본체번호를 PC(SeePro)가 G1 으로 내려준 값으로 기록.
 *         G1 이 0(=`0000`)이거나 미수신이면 기기 자체 번호를 쓴다(사용자 확정).
 *         1.0 은 G1 구간을 읽지 않고 항상 기기 번호를 썼다. 홈 화면 우하단은
 *         **실제 기록될 번호**를 표시한다.
 * 2.2.5 — 전체 로직 감사 반영. 핵심:
 *         · 세션 도중 StopCrypto1 금지 규칙을 Read 실패·태그 발급 경로까지
 *           확대(암호화 상태를 mCryptoOn 으로 분리 추적). 서버 덤프가 1회
 *           읽기 실패 후 전량 0 으로 나가던 것, 태그 발급(type 0/2)이 항상
 *           실패하던 것 수정. Read 는 nested 재인증 1회 재시도.
 *         · 기록 커밋 순서 정정(소독 Process·게이트웨이 Status 를 마지막에)
 *           → 중간 실패 시 "했다고 표시되지만 기록은 빈" 태그가 남지 않음.
 *         · 소독 횟수 오계상 2건: 더블터치 재시작 중복 증가, guest 슬롯 잔존.
 *         · 메뉴 숫자 버퍼 오버런(값 100 이상), abort() 영구정지 → 소프트리셋,
 *           기기 타입 EEPROM 손상 시 기본값 폴백, 동시소독 지연 상한 클램프.
 *         · 응답성: Home 화면 변경분만 LCD 갱신, 시리얼 수신 타임아웃 1s→150ms
 *           (두 G 패킷 병합으로 옛 환자정보가 채택되던 위험도 함께 축소).
 *         · 진단: 태그 읽기 실패 사유 표시, legacy 덤프 읽기 실패 경고.
 *         · 도장 기준을 빌드 시각 → **버전 문자열**로 변경(결정적 규칙:
 *           버전이 오른 펌웨어를 올리면 완전 초기화, 같은 버전은 설정 유지).
 */
#define TRACEQ_VERSION_MAJOR 2
#define TRACEQ_VERSION_MINOR 2
#define TRACEQ_VERSION_PATCH 20
#define TRACEQ_VERSION_STRING "2.2.20"
// 이 판을 만든 날 — 시계가 이보다 앞서면 실제 시각일 수 없다(BaseRtc::IsUnsynced). 버전을 올릴 때 같이 올린다.
// ★출시일은 **빌드한 날 또는 과거**여야 한다 — 미래로 적으면 모든 기기가 영구 '미동기'(IsUnsynced) 가 되어 액교환일이
//  옛날로 찍힌다. 시험은 이 값에서 파생(rel_date)하므로 못 잡는다 → 아래 static_assert 가 컴파일 날짜(__DATE__)와 대조해
//  미래면 빌드를 실패시킨다(5차 재검증 V1 — 내가 27 로 올렸는데 그날은 26 이었다).
#define TRACEQ_RELEASE_YEAR  2026
#define TRACEQ_RELEASE_MONTH 9
#define TRACEQ_RELEASE_DAY   26

// __DATE__ = "Mmm dd yyyy" (dd 는 공백 채움). C++11 constexpr(단일 return).
constexpr int tq_build_month()
{
    return (__DATE__[0] == 'J' && __DATE__[1] == 'a') ? 1 : (__DATE__[0] == 'F') ? 2
         : (__DATE__[0] == 'M' && __DATE__[2] == 'r') ? 3 : (__DATE__[0] == 'A' && __DATE__[1] == 'p') ? 4
         : (__DATE__[0] == 'M') ? 5 : (__DATE__[0] == 'J' && __DATE__[2] == 'n') ? 6 : (__DATE__[0] == 'J') ? 7
         : (__DATE__[0] == 'A') ? 8 : (__DATE__[0] == 'S') ? 9 : (__DATE__[0] == 'O') ? 10 : (__DATE__[0] == 'N') ? 11 : 12;
}
constexpr int tq_build_day()  { return (__DATE__[4] == ' ' ? 0 : (__DATE__[4] - '0') * 10) + (__DATE__[5] - '0'); }
constexpr int tq_build_year() { return (__DATE__[7] - '0') * 1000 + (__DATE__[8] - '0') * 100 + (__DATE__[9] - '0') * 10 + (__DATE__[10] - '0'); }
constexpr long tq_day_index(int y, int m, int d) { return y * 372L + m * 31L + d; }
static_assert(tq_day_index(TRACEQ_RELEASE_YEAR, TRACEQ_RELEASE_MONTH, TRACEQ_RELEASE_DAY)
              <= tq_day_index(tq_build_year(), tq_build_month(), tq_build_day()),
              "TRACEQ_RELEASE_* is later than the build date - set it to today or earlier");
// 1.0 UI가 사용하던 매크로 이름 — 호환을 위해 별칭 유지.
#define TRACEQ_ARDUINO_VERSION TRACEQ_VERSION_STRING
