...
int main()
{
    rtc_set(DateTime(2026, 9, 23, 7, 0, 0));
    boot('D');
    make_tag(mgr, 0x01, MANAGER_TYPE_TAG, 7, "ND01456", "KIMJH");
    touch(mgr);
    make_tag(clr, 0x02, CLEAR_TYPE_TAG, 0, "", "");
    touch(clr);                                            // 정상 시계에서 교환(09-23 07:00)
    CHECK(clear_is(2026, 9, 23, 7, 0), "회귀: 시계 정상 → 교환일 즉시 기록");
    ...
    json("{\"cmd\":\"cfg_set_date_time\",\"device_date_time\":\"2026-09-23 09:30:00\"}");
    run_loops(2);
    CHECK(clear_is(2026, 9, 23, 9, 30), "DTR 리셋 뒤 시계 맞춤 → 그 시각이 교환일");
    ...
    rtc.SetDateTime(DateTime(2026, 9, 23, 9, 45, 0));     // 날짜까지 맞춤
    run_loops(2);
    CHECK(clear_is(2026, 9, 23, 9, 45), "날짜까지 맞춤 → 그 시각이 교환일");
