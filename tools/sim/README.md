...
- 시뮬레이터 RAM 은 32KB — `SimCard`(≈1.2KB)를 정적으로 열 장 넘게 만들면 스택이 넘쳐 0 번지 재시작을 반복한다(카드는 돌려쓰기).
...
1. "정지시키지 못한 카드는 REQA 에 한 번 걸러 응답한다" — 실물이 3회 연속 무응답이면 디바운스 판정이 뒤집힌다.
2. "인증 실패 → 카드 IDLE" — 아니면 발급 '먼저 올려 둠' 이 timeout 이 된다.
